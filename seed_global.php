...
// 2. Clean EVERYTHING to avoid conflicts/duplicates
...
function addSched($db, $roomName, $day, $start, $end, $subCode, $section, $facName, $uid, $tid) {
    // 1. Ensure Room (Global or per user? Default config has no user_id on rooms, but I'll check)
    // Actually config.php rooms table has NO user_id.
    ...
    // 2. Ensure Subject (Per user probably, as per teacher_schedule.php query)
    // Wait, subjects table in config.php has no user_id? 
    // Wait, config.php line 71: ALTER TABLE subjects ADD COLUMN curriculum_id ...
    // Let's check check_cols.php output: subjects has id, code, name, units, curriculum_id, term_id. No user_id.
    ...
    // 3. Ensure Faculty (Per user as per teacher_schedule.php query)
...
foreach ($users as $uid) {
        ...
        foreach ($roomSched as $dayGroup) {
            ...
            for ($i = 1; $i < count($dayGroup); $i++) {
                ...
                foreach ($days as $day) {
                    ...
                    // Specific logic for 3rd slot and onwards to handle lunch break gap
                    // In user's table, row 3 is 1:00-3:00 (13:00-15:00)
                    // My times[2] is 12:00, times[3] is 13:00.
                    // So i=1 (8-10), i=2 (10-12), i=3 (13-15?), i=4 (15-17), i=5 (17-19), i=6 (19-21)
                    if ($i >= 3) {
                        $start = $times[$i];   // i=3 -> times[3]=13:00
                        $end = $times[$i+1];   // i=3 -> times[4]=15:00
                        if (!isset($times[$i+1])) $end = date('H:i:s', strtotime($start . ' +2 hours'));
                    }
                    
                    addSched($db, $roomName, trim($day), $start, $end, $parts[0], $parts[1], $parts[2], $uid, $term_id);
                }
            }
        }
    }
    ...
    // Extra entries
    addSched($db, "CISCO (CON 102)", "Tuesday", "17:00:00", "19:00:00", "IT-126", "AI34", "TIQUEN", $uid, $term_id);
    addSched($db, "CISCO (CON 102)", "Friday", "17:00:00", "19:00:00", "IT-126", "AI34", "TIQUEN", $uid, $term_id);
    addSched($db, "CISCO (CON 102)", "Tuesday", "19:00:00", "21:00:00", "IT-126", "AI33", "TIQUEN", $uid, $term_id);
    addSched($db, "CISCO (CON 102)", "Friday", "19:00:00", "21:00:00", "IT-126", "AI33", "TIQUEN", $uid, $term_id);
}

echo "SUCCESS: Full global seeding completed for all users!\n";
?>
